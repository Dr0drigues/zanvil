---
title: Configuration
description: Configurer zanvil (zsh_env) via config.zsh et les modules
---
...
Les modules sont maintenant gérés via `zsh-env-modules` :
...
Le fichier `.zsh-env.local` dans un répertoire est auto-chargé lorsque vous y entrez (similaire à direnv). Un mécanisme de confiance basé sur un hash empêche l'exécution de fichiers non approuvés.

```bash
# Créer un fichier local
echo 'export NODE_ENV=development' > /path/to/project/.zsh-env.local

# La première fois, zsh-env demandera d'approuver le fichier
...
Les profils sont stockés dans `~/.zsh_env/profiles/`.
...
Le thème actif est enregistré dans `~/.zsh_env/.current_theme`.
