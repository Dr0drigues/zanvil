...
- Env : `ES_URL` puis `ZANVIL_WORK_ES_URL` (défaut `https://hote-interne`) ; `ES_USER`/`ES_PASSWORD` requis avant tout appel ; index `${ZANVIL_WORK_ES_INDEX:-es-apis-*}` ; `SSL_CERT_FILE` → `--cacert`.
...
[[ $(_work_es_url) == "https://hote-interne" ]] || { echo "FAIL url defaut"; exit 1 }
